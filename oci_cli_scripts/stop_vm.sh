...
sh $SCRIPT_DIR/check_vm_status.sh $VM_NAME STOPPED
ret=$?
...
oci compute instance action --instance-id ${VM_OCID} --action STOP --profile $profile
...
while (( attempt <= MAX_ATTEMPTS )); do
  ...
  echo "Waiting for 30 secs for VM to STOP"
  sleep 30s
  ...
  if [ $ret -eq 0 ];
  then
    echo "VM $VM_NAME is successfully STOPPED"
    success="true"
    break
  fi
