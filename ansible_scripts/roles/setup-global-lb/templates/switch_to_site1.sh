...
echo "IP Table rules before switching to site1"
#view port forwarding rules
iptables -t nat -L -v --line-numbers

iptables -w -t nat -D PREROUTING -p tcp --dport {{ ohs_http_port }} -j DNAT --to-destination {{site2_lb_vm_ip}}:{{ ohs_http_port }}
iptables -w -t nat -D PREROUTING -p tcp --dport {{ ohs_https_port }} -j DNAT --to-destination {{site2_lb_vm_ip}}:{{ ohs_https_port }}
iptables -w -t nat -D PREROUTING -p tcp --dport {{ ohs_http_port_for_wls_admin_server }} -j DNAT --to-destination {{site2_lb_vm_ip}}:{{ ohs_http_port_for_wls_admin_server }}
...
echo "IP Table rules after switching to site1"
#view port forwarding rules
iptables -t nat -L -v --line-numbers
