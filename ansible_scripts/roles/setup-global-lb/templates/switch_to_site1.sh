...
sudo iptables -t nat -A PREROUTING -p tcp --dport {{ohs_http_port}} -j DNAT --to-destination {{site1_lb_vm_ip}}:{{ohs_http_port}}
sudo iptables -t nat -A PREROUTING -p tcp --dport {{ohs_https_port}} -j DNAT --to-destination {{site1_lb_vm_ip}}:{{ohs_https_port}}
sudo iptables -t nat -A PREROUTING -p tcp --dport {{ohs_http_port_for_wls_admin_server}} -j DNAT --to-destination {{site1_lb_vm_ip}}:{{ ohs_http_port_for_wls_admin_server }}
